...
def create_vote(update, context):
    ...
    del args[0]
    answer_options = args
    keyboard = ptbh.get_inline_keyboard_markup(answer_options,CallbackOptions.VOTE.value, 1)
    message_text = ptbh.write_vote(question, answer_options)
    vote = Vote(datetime.datetime.utcnow(), question, answer_options, message_text)
    write_vote_to_DB(id, vote)
    message = update.message.reply_html(message_text, reply_markup=keyboard, quote=False)
    chat_type = update.message.chat.type
    if chat_type == "supergroup" or chat_type == "group":
        try:
            context.bot.pin_chat_message(message.chat.id, message.message_id, disable_notification = True)
        except BadRequest:
...
def handle_vote_callback(bot, update, query, answer):
    ...
    result = ptbh.rewrite_vote(vote.result, answer, vote.answer_options, username)
    keyboard = ptbh.get_inline_keyboard_markup(vote.answer_options, CallbackOptions.VOTE.value, 1)
    try: 
        ...
        notification = ptbh.draw_answer(msg.VOTE_NOTIFY).format(vote.answer_options[answer])
    except BadRequest:
