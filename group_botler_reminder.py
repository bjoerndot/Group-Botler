    ...
    try:
        ...
        if ":" in input_value:
            ...
            if input_value.endswith("UTC"):
                ...
                seconds = (remind_time - datetime.datetime.utcnow()
                           ).total_seconds()
            ...
            for key, value in jobs.items():
                ...
                if datetime.datetime.utcnow() > value.end_date and not value.repeating:
                    ...
                    if value.callback == c.RUN_REMINDER:
                        ...
                        job_queue.run_once(
                            run_reminder, value.end_date, context=job, name=str(key))
                    elif value.repeating:
                        next_execution = value.next_execution()
                        job = Job(value.id, 0, value.text, value.name)
                        job_queue.run_repeating(
                            run_repeating_reminder, value.interval, next_execution, context=job, name=str(key))
                    else:
                        print("Could't schedule job {}: Missing function to call.".format(
                            value["name"]))
                        to_delete.append((user, key))
...
def list_reminders_summary(update, context):
    ...
    reminder_message = create_reminder_list_message(
        context.job_queue, chat_id, chat_name)
...
def list_reminders(update, context, chat_id=0):
    ...
    if not chat_id:
        ...
        chat_name = ptbu.get_group_title(update)
    reminder_message = create_reminder_list_message(
        context.job_queue, chat_id, chat_name, False)
    ptbh.reply(update, reminder_message, quote=False)
...
def show_reminder(update, context):
    ...
    if reminder.reply_to:
        context.bot.send_message(
            chat_id=chat_id, text=reminder.name, reply_to_message_id=reminder.reply_to)
    else:
...
def add_reapeting_reminder(update, context):
    ...
    setup_repeating_reminder(context.bot, update, context.job_queue,
                             update.message.chat.id, name, interval, start_time, message_text)
...
def setup_reminder(bot, update, job_queue, chat_id, message_id, name: str, timedelta: datetime.timedelta, abs_time=False, dt=0):
    ...
    reminder = Reminder(name, remind_time, "run_reminder",
                        message_id, chat_id, False, name)
...
def setup_repeating_reminder(bot, update, job_queue, chat_id, name, interval, start_time, text):
    ...
    job_queue.run_repeating(callback=run_repeating_reminder,
                            interval=interval, first=start_time, context=job, name=name)
    reminder = Reminder(str(name), start_time,
                        "run_repeating_reminder", 0, chat_id, True, text, interval)
    add_reminder_to_storage(chat_id, reminder)
    update.message.reply_html(msg.R_ADD_JOB_SUCCESS.format(name))
...
def run_reminder(context):
    ...
    context.bot.send_message(chat_id=job.context.id, text=job.context.text,
